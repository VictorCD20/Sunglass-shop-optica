...
import { CONTACT, NAV, WHATSAPP_MESSAGES, whatsapp } from "@/lib/site-data";
...
export function SiteFooter() {
  const handlePhoneClick = () => {
    trackEvent({ event: "click_phone", event_category: "lead", event_label: "phone_call" });
  };

  const handleWhatsappClick = () => {
    trackEvent({ event: "click_whatsapp_general", event_category: "lead", event_label: "whatsapp_general" });
  };
  ...
  return <>
    <footer className="footer">
      ...
      <div>
        <h3>Contacto</h3>
        <p><MapPin size={16} /> {CONTACT.address}</p>
        <a href={`tel:${CONTACT.phoneDigits}`} onClick={handlePhoneClick}>{CONTACT.phone}</a>
        <a href={whatsapp(WHATSAPP_MESSAGES.general)} onClick={handleWhatsappClick} target="_blank" rel="noreferrer">WhatsApp directo</a>
      </div>
    ...
    <a className="whatsapp-float" href={whatsapp(WHATSAPP_MESSAGES.general)} onClick={handleWhatsappClick} target="_blank" rel="noreferrer" aria-label="Contactar por WhatsApp"><MessageCircle /></a>
  </>;
}
