...
export function CatalogExperience() {
  ...
  const items = useMemo(() => COLLECTIONS[brandId] ?? [], [brandId]);
  return <section id="catalogo-marcas" className="catalog-experience">
    ...
    <div className="brand-selector" role="tablist" aria-label="Marcas de lentes">{BRANDS.map((item) => <button key={item.id} role="tab" aria-selected={brandId === item.id} className={brandId === item.id ? "active" : ""} onClick={() => setBrandId(item.id)}>{item.logoDark ? <img src={item.logoDark} alt={item.name}/> : <span className="brand-name-mark brand-name-mark--dark">{item.name}</span>}</button>)}</div>
    <FeatureCarousel key={brandId} items={items} brand={brand.name} />
    {!items.length && <p className="availability-note">La colección {brand.name} está disponible en nuestra sucursal. Consulta modelos y disponibilidad directamente con nuestro equipo.</p>}
